...
def add_cone(polygons, cx, cy, cz, r, h, step):
    ...
    for rotation in range(rot_start, rot_stop):
        rot0 = rotation/float(step)
        rot1 = (rotation + 1)/float(step)
        x0 = r*math.cos(2*math.pi*rot0) + cx
        y0 = r*math.sin(2*math.pi*rot0) + cy
        x1 = r*math.cos(2*math.pi*rot1) + cx
        y1 = r*math.sin(2*math.pi*rot1) + cy
...
def add_cylinder(polygons, cx, cy, cz, r, h, step):
    ...
    for rotation in range(rot_start, rot_stop):
        ...
        add_polygon(polygons, cx, cy, cz,
                        x0, y0, cz,
                        x1, y1, cz)
        add_polygon(polygons, x0, y0, cz + h,
                        x0, y0, cz,
                        x1, y1, cz)
        add_polygon(polygons, x1, y1, cz,
                        x1, y1, cz + h,
                        x0, y0, cz +h)
        add_polygon(polygons, cx, cy, cz + h,
                        x0, y0, cz + h,
